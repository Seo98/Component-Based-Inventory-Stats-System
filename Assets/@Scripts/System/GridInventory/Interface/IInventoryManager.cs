...
public interface IInventoryManager : IDisposable
{
    ...
    /// </summary>
    event Action<IInventoryItem> onItemAddedFailed;
    ...
    /// </summary>
    event Action<IInventoryItem> onItemDroppedFailed;

    /// <summary>
    /// Invoked when an item remains in the inventory but its runtime state changes.
    /// </summary>
    event Action<IInventoryItem> onItemChanged;

    /// <summary>
    /// Invoked when the inventory is rebuilt from scratch
    /// </summary>
    event Action onRebuilt;
    ...
    /// <summary>
    /// Notifies observers that an existing item's mutable state changed.
    /// </summary>
    void NotifyItemChanged(IInventoryItem item);
